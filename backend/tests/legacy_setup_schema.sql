-- The schema script LaunchOps ran at every startup before Alembic migrations (database.SETUP_SQL,
-- frozen when migrations replaced it). tests/test_migrations.py builds a database with it to prove the
-- migrations reproduce that schema and upgrade databases it created. Never edit it.
-- Products
...
-- One-time carry-over: registration used to be switched off on the settings rows
