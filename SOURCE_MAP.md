...
Last updated 2026-09-19 (after pull request #6). For how to run, see `CLAUDE.md`; for tests, `docs/TESTING.md`;
...
| `docs/` | Managed documents: `ROADMAP.md`, `BUGS.md`, `TESTING.md`, `HANDOFF.md`, `CHANGELOG.md`, `AUDIT-LOG.md`; plus the assessment and development plan, `PHASE1_DESIGN.md`, `SHARING_PLAN.md` (the sharing brief) and the design system. |
...
| `routers/queue.py` | `/api/queue`: list/get (Viewer), a project's results counted by operation and status over all of them (`/api/queue/summary`, Viewer; the playbook's record of what's finished), review (Approver), cancel a waiting or running operation (Editor: 200 cancelled, 202 stopping, 409 already finished), delete (Approver). Approving copies email addresses into drafts (never sends). `/api/email-queue`: list, edit draft (Editor; 409 once sent), quota, send one (Approver; explicit; 429 at the daily limit), delete (Approver). Changes are published as live updates and governed actions go to the activity log. |
...
| `services/usage.py` | The AI usage ledger: a row per API response; the monthly budget check against the budget that applies, the organisation's own or the platform default (`effective_budget`); who may set which budget (`ensure_budget_allowed`, which `set_budget` applies with the organisation's row locked); and a month's totals by operation, project, member and model. |
...
| `queries/hooks.ts`, `queries/keys.ts` | Every data hook: optimistic checklist saves, calendar moves and launch-date changes; queue polling while work runs (every 4 s, or every 30 s while live updates are connected), for the results and for the summary the playbook reads; cancelling operations; usage and budget; undoable deletes. |
...
| `domain/playbook.ts` | The launch playbook: seventeen operations in five ordered stages, each with its purpose, T-minus window and the stages it builds on. From a project and its results (`ResultRecord`: a full result or a line of `/api/queue/summary`) it works out each operation's state, each stage's progress and status, the current stage and the one operation to run next. It advises and never blocks; the one tool sits apart in `ALWAYS_AVAILABLE`. |
...
| `domain/usage.ts` | Usage months (UTC), dollar and count formats, budget use (warn from 80%, used up at 100%), budget input parsing, and whether the viewer can raise the budget that applies (`canRaiseBudget`, the backend's rule) so the page never promises a raise they can't make. |
...
| `operations/Playbook.tsx`, `Playbook.module.css` | The Operations screen's default view: a "Next up" card with the one operation to run now and why (behind its window, waiting on review, or everything done), then the five stages as an ordered list — the current one open, the rest one line each, and a stage opens when it becomes current — and "Always available" for the tool. |
| `operations/OperationCard.tsx` | One operation: what it produces, its kind and how it runs, its progress in the playbook, and the controls to open its report or run it (Run for Editors and above). Shared by the playbook and "All operations", so an operation reads the same in both. |
...
| `project/` | Project layout and tabs: Overview, Operations (the launch playbook, or all operations by category with `?view=all`), Reports (index and document with export), Launch plan, Settings. |
...
| `e2e/a11y.spec.ts` | axe-core WCAG 2.2 A/AA scans of 28 enumerated screens in light and dark themes, including both Operations views, a research result and a report with their Sources sections. |
| `e2e/responsive.spec.ts` | The same 28 screens at a 390px phone width: fails when the document is wider than the viewport, and names the element that makes it so. |
