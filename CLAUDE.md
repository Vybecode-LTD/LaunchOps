...
| **Changelog / audit log** | `docs/CHANGELOG.md` — what each version of the documentation set changed, newest first; the top entry is where things stand (1.1.3: pull request #5, not merged when this was written, up to its last code commit `7dbc35d` — the default AI budget and the rule that only a platform admin may go above it (BUG-028, BUG-031), a cap per organisation rather than on the total (B-14); four screens fixed at phone width (BUG-029); empty competitor columns (BUG-030); the playbook domain module; the automated review's fixes, with BUG-032 left open; a fourth audit; and gap 14, the Vitest suite unreliable at default concurrency on this machine) / `docs/AUDIT-LOG.md` — the reconciliation audits, newest first |
...
- **Active task:** **pull request #5, open and awaiting merge** — branch `fix/spend-cap-and-mobile-overflow`, not merged and not deployed. Its commits up to `7dbc35d`, oldest first, all pushed: `c7358fb` the 1.1.2 documentation reconciliation, `522e40d` the default AI budget, `2d57b35` the phone-width overflow fixes and the responsive suite, `a2124a9` documenting the budget and the responsive suite, `597e94b` the empty competitor columns, `8c9f2e2` the playbook domain module, `908f46d` the launch plan checklist test, two review fixes — `6b97da3` (a re-run no longer reopens a finished playbook stage) and `43ab3e6` (Settings → Usage shows the budget that actually applies; a negative default is refused) — **`a53b26e`, the BUG-031 fix**, and **`7dbc35d`, the last code commit on pull request #5**: the pull request's code is frozen there. `7dbc35d` answers CodeRabbit's review of `43ab3e6`: Settings → Usage now states which budget applies (the platform default, the organisation's own, or none) even when the current month has no AI usage yet. Before it, the "No AI usage" empty state replaced the month's summary, the only place the budget appeared, so a brand-new organisation saw no sign of its $25 cap until an operation had already cost something. It also changes the budget field's hint from "until an owner raises it" to "until the budget is raised" and corrects a test docstring that still called the default "not a ceiling" — its only backend change — and adds two frontend tests, written to fail first. The documentation that records `a53b26e` and `7dbc35d` is committed and pushed on pull request #5, after `7dbc35d`, and the merge waits for CI to pass on the pull request's latest commit (`gh pr checks 5`). Registration makes every new account the Owner of its own organisation, and an owner could set any budget, so until `a53b26e` the default held only organisations that never touched their budget, and one request lifted it. Now, by the owner's decision of 2026-09-17, owners can set any budget up to the default or clear it, and only a platform admin can set one above it — though an owner cannot yet lower a budget stored above the default to an amount still above it (BUG-032, under Open bugs). **Merging pull request #5 is what caps the live spending exposure for self-registered accounts — per organisation, not in total:** every push to `main` deploys, and until pull request #5 is merged and deployed, production runs `bc6143c`'s code, with open registration and **no default cap**, so an organisation that has not set a budget of its own can spend on the deployment's one `ANTHROPIC_API_KEY` without a cost cap. Once the merge deploys, every organisation that registration creates is held to the $25 default, which its owner can lower or clear but not raise. That bounds each organisation, not the key as a whole: with open registration on (T-2), every account someone registers — up to ten an hour from one network address — comes with its own organisation and its own $25, and nothing caps the total (B-14). A budget already stored above the default is kept, because the rule applies to changes. **`main` is at `0ce65dd`, the merge of pull request #4** (branch `docs/record-the-merge`, two commits: `9f7e930` recording the merge, the deployment and a second audit, `6a54c04` pointing the changelog row at the newest entry) — documentation only: it touched `CLAUDE.md` and `docs/` alone, no application code, no tests, no CI config. So **`bc6143c` is still the last commit on `main` that changed application code, and what production runs is unchanged in behaviour.** Before that, the BUG-027 work merged as **pull request #3** (branch `fix/refresh-token-clock-skew`, six commits: `35d24c5` the BUG-027 refresh-token fix, `584cff6` the text-block regression test, `7c1f1cb` the session-end documents, `39ff28b` the design-system correction, `ee4932a` the reconciliation fixes, `c486949` the branch-state notes), after all three CI jobs passed: backend lint and tests (550 tests, the 95% coverage gate, against a `postgres:18` service), frontend lint/typecheck/Vitest/build/Playwright, and the gitleaks secret scan. It was merged with a **merge commit, not a squash**, so the `.gitleaksignore` fingerprints still resolve. Railway deployed `bc6143c` at 2026-09-17T19:47:08Z, so **the BUG-027 refresh-token fix is now in production.** Only `.github/workflows/test-pipeline.yml` remains deliberately untracked.
- **Next** (priority order; the first two are owner-only account and console work, not code — full list in `docs/ROADMAP.md` → Active):
  ...
  - **rotate the Railway project token at session end (T-4)** — the owner's settled practice: `railway login` will not authorise on this machine, so the owner pastes a project token into the session and rotates it at the end of every session, which bounds the exposure. The token in use now was pasted into chat like the one before it, so it is owed that rotation. Never write a token value into a file, a document or a commit message: CI runs gitleaks over the full git history.
  ...
  - owner decisions: plan section 8; the brand kernel questions in D15; whether organisation owners should also create reset links (D8); and, from pull request #5's budget work, a cap on total AI spend across organisations (B-14) and whether a platform admin should be able to set the budget of an organisation they don't belong to (B-15)
  - Phase 2 follow-up: brand kernel, result history, billing settings
  - Phase 3: real actions
- **Open bugs:** 1, **BUG-032 (LOW)**, from CodeRabbit's review of `7dbc35d`: `usage.ensure_budget_allowed` compares a requested budget with the platform default, not with the organisation's current budget, so where a platform admin has set an organisation's budget to, say, $500, an owner who isn't a platform admin cannot lower it to $400 — only to the default ($25) or below, or clear it. It fails safe: it can refuse a decrease the owner's rule allows, but never allows a budget that rule forbids. It came in with `a53b26e`, so it is not in production, but it is not fixed on pull request #5 either — the code there is frozen — so the merge will ship it. The record is `docs/BUGS.md`: 32 bugs, 31 of them fixed. BUG-001 to BUG-027 are on `main`; **BUG-028 to BUG-031 are fixed on pull request #5's branch only, so none of the four is on `main` or in production yet.** BUG-028 to BUG-030 shipped, and stay live in production until the pull request merges and deploys; BUG-031 was never live, because the default cap it could lift exists only on the branch. Every fix landed with a test that failed first except BUG-023's, a `.gitignore` change verified by a tracking check instead. Three known limitations (LIM-001 to LIM-003). Remaining findings and phase status are in `docs/ASSESSMENT_AND_DEVELOPMENT_PLAN.md`.
- **Doc version:** 1.1.3 — this is the `version:` in this file's frontmatter, not a separate number. All seven managed documents carry that one shared version and are raised together.

**Tests (2026-09-17, measured on branch `fix/spend-cap-and-mobile-overflow` at `7dbc35d`)**

| Suite | Result | Coverage |
|---|---|---|
| Backend pytest | 564 passed | 98.32% of application code — 3,340 statements, 56 missed (tests excluded). `backend/.coveragerc` enforces the 95% deploy gate (`fail_under = 95`; omits `tests/`, `.venv/` and `venv/`) whenever coverage is collected (`python -m pytest --cov`, as CI runs it). |
| Frontend Vitest | 50 files, 628 passed | 99.21% lines (3,268 of 3,294); statements 97.08% (3,761 of 3,874), branches 90.13% (3,032 of 3,364), functions 96.53% (1,478 of 1,531). `frontend/vitest.config.ts` enforces 95% lines (`coverage.thresholds`) in `npm run coverage`. |
...
│   ├── e2e/                   # smoke, golden, a11y and responsive specs (responsive.spec.ts: 27 enumerated screens at 390px —
...
- **Usage and budgets:** every Claude API response goes in the `ai_usage` ledger with an estimated cost, including $10 per 1,000 web searches. A monthly budget per organisation gives 429 once reached, and an organisation without one of its own — never set, or cleared — falls back to the platform default, `DEFAULT_MONTHLY_AI_BUDGET_USD`. **Owners can set any budget up to the default or clear it; only a platform admin can set one above it** (`ensure_budget_allowed()` on `PUT /api/organisation/budget`, 403 otherwise; no ceiling while the default is 0), and an owner can lower a budget stored above the default only to the default or below (BUG-032, open). The route is Owner-only either way, so an admin can go above the default only in an organisation where they are an Owner. The 429 names whoever can raise the budget: an owner while the organisation's own budget is under the ceiling, otherwise a platform administrator. Settings → Usage shows owners the budget that actually applies — the organisation's own, the platform default or none — because enforcement and the usage summary both go through `effective_budget()` in `backend/services/usage.py`. Where it says so depends on the month. In a month with AI usage, the month summary shows it beside the costs, with a meter when there is a budget and, only while the platform default is the one that applies, the note "This is the platform's default budget. You can set a lower one below; only a platform administrator can set a higher one." In the current month with no usage yet, as in a brand-new organisation, the empty state is followed by one sentence saying which budget applies — on the default, "The platform's default budget of $25.00 applies: operations stop when a month's cost reaches it." — and there is no note about administrators (`BudgetStatement` in `frontend/src/pages/settings/UsageSettings.tsx`). In a past month with no usage, the month panel shows only the empty state.
...
| `DEFAULT_MONTHLY_AI_BUDGET_USD` | backend | Monthly AI spending cap for **any organisation that has not set a budget of its own** (default 25), and **the most an owner can set**. Registration creates an organisation with no budget, and every deployment bills AI to one `ANTHROPIC_API_KEY`, so without this a self-registered account could spend on that key without limit. An organisation's own budget wins over the default, but **only a platform admin can set one above it**: owners can set any budget up to it or clear theirs (`ensure_budget_allowed()` in `backend/services/usage.py`, 403 otherwise), and a budget already stored above it is kept, because the rule applies to changes — though an owner can lower such a budget only to the default or below (BUG-032, open). **Clearing** an organisation's budget falls back to this default rather than meaning unlimited. It is **validated as non-negative** (`ge=0` in `backend/config.py`): a negative value fails at startup rather than silently removing the cap, and **only exactly `0`** opts out, meaning no default cap and no ceiling on what an owner can set. **Settings → Usage shows the budget that actually applies** — the organisation's own, or the platform default, labelled as such — because the usage summary returns `effective_budget_usd` and `budget_source` (`organisation`, `default` or `none`) alongside the organisation's own `budget_usd` (which the budget form edits) and `default_budget_usd`. Documented in `backend/.env.example`. |
...
`docs/ROADMAP.md` is the task list: **Active** (T-1 to T-9: T-9, merging pull request #5, comes first; T-1 to T-8 are the owner-only follow-ups — two were settled on 2026-09-17: T-2, the admin account and the registration policy, and T-3, the Spaceship CNAME; T-4 stays open at P1, because the token issued to replace the exposed one was exposed the same way), **Blocked / needs the owner** (fourteen open: B-1 to B-12, and B-14 and B-15, which came out of the budget work in pull request #5), **Decided** (B-13, the session-end merge, done 2026-09-17) and **Next up**. B-14 asks whether anything should cap total AI spend across organisations: once pull request #5 merges, each organisation is capped but nothing caps the total, and every open sign-up creates an organisation with its own $25 a month. B-15 asks whether a platform admin should be able to set the budget of an organisation they don't belong to: as built they can only where they are an Owner, yet the 403, the 429 and the note on Settings → Usage all name a platform administrator as the one who can go higher. `docs/HANDOFF.md` is the last session's handoff — read it at session start. The **"Progress"** section of `docs/ASSESSMENT_AND_DEVELOPMENT_PLAN.md` has the phase status, the findings and the reasoning; the open owner questions on the brand kernel (D15) and reset links (D8) are in `docs/PHASE1_DESIGN.md`. Every session ends with **"perform handoff"**.

---

Last-verified: 2026-09-18T03:08Z (23:08 on 2026-09-17 on the owner's clock, UTC-4) · branch `fix/spend-cap-and-mobile-overflow`, all pushed, on **pull request #5, open and not merged**; its last code commit is `7dbc35d` (Settings → Usage states which budget applies before anything has run), and the documentation recording `a53b26e` and `7dbc35d` is committed and pushed on the same pull request · `main` = `origin/main` at `0ce65dd` (the pull request #4 merge, documentation only) · production runs `bc6143c`'s application code (the pull request #3 merge, deployed 2026-09-17T19:47:08Z), so neither the default AI budget nor the ceiling on what an owner can set is live · merging pull request #5 caps the spending exposure per organisation, not in total (B-14): each self-registered organisation is held to the $25 default, which its owner can lower or clear but not raise, and only a platform admin can set a budget above it · `.github/workflows/test-pipeline.yml` stays untracked by design · confirm with `git status`, `git log origin/main..`, `gh pr view 5` and `gh pr checks 5`
