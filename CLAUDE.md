...
document: CLAUDE
version: 1.2.1
last-updated: 2026-09-19T17:06:07Z
...
| **Changelog / audit log** | `docs/CHANGELOG.md` — what each version of the documentation set changed, newest first; the top entry is where things stand (1.2.1: the 1.2.0 handoff merged, T-10's signed-in check and T-1 done by the owner, every merged branch deleted, and Codex's two points folded into the sharing plan) / `docs/AUDIT-LOG.md` — the reconciliation audits, newest first |
...
- **Last completed task:** **the 1.2.0 handoff merged as `c8da4f0`; the owner finished T-10's signed-in check and T-1; and every merged branch was deleted** — all on 2026-09-19, after pull request #6 had merged, deployed and been verified on `main`.
  - **The 1.2.0 handoff merged.** Pull request #7 (branch `docs/handoff-2026-09-19`, commit `101f706`) merged into `main` as the merge commit `c8da4f0` at 2026-09-19T16:27:03Z, with the owner's go-ahead, after all three CI jobs passed on `101f706`; CodeRabbit's review was still pending, and it is not part of CI. CI passed again on the push to `main` (run 35454992066). The merge carried documentation only, so production behaves as before and its application code is still `199121c`'s, checked from the live site at 2026-09-19T16:32:19Z ([Deployment & CI](#deployment--ci)). Codex reviewed pull request #7 with two P2 comments on `docs/SHARING_PLAN.md`; both are folded into the plan (see the active task), and neither is answered on GitHub, because posting there waits for the owner's go-ahead.
  - **The owner's tasks, done on 2026-09-19 after that merge.** **T-10 is done:** the owner did its signed-in check on the live site and reported that it looks good — the Operations screen opens on the playbook, and Settings → Usage is right. That is the owner's report; the assistant does not sign in. **T-1 is done:** the owner deleted the stray account `guard-check@example.com` and "Guard check's organisation". **T-4:** the owner reported rotating the Railway project token at the end of the 2026-09-19 session; it recurs at the end of every session (see Next).
  - **Every merged branch deleted.** On 2026-09-19, at the owner's request, every merged branch was deleted, locally and on `origin`, after checking that each was fully merged into `main`, unprotected, and used by no open pull request or workflow: `feature/launchops-v2-foundation` (#1), `chore/docs-deployment-status` (#2), `fix/refresh-token-clock-skew` (#3), `docs/record-the-merge` (#4), `fix/spend-cap-and-mobile-overflow` (#5), `feat/playbook-ui` (#6) and `docs/handoff-2026-09-19` (#7). The pull requests keep their commits, and GitHub can restore any branch. **Only `main` remains**, apart from this 1.2.1 update's own branch, which is deleted like the others once it merges. Branch names elsewhere in these documents stay as the record of where work was done.
  - **Pull request #6 merged** into `main` as `199121c` at 2026-09-18T19:08:32Z, with the owner's go-ahead — a merge commit, not a squash — after all three CI jobs passed on its last commit, `705c834` (the 1.1.4 documentation; green by 2026-09-18T06:43:43Z). CI passed again on the push to `main` (run 35384260825). `199121c`'s tree is identical to `705c834`'s, so the code on `main` is exactly the code measured at `b607021`. Production runs `199121c`'s application code, checked from the live site rather than the Railway dashboard (see [Deployment & CI](#deployment--ci)).
...
- **Active task: sharing — the owner wants to start on it immediately in the next session.** On 2026-09-19 the owner asked whether projects can be shared. Today sharing is organisation-wide only: an Owner invites someone by email or link with a role (Viewer, Editor, Approver or Owner), the invitation lasts 7 days, and the person then sees every project in the organisation. There is no per-project access, no view-only share link, and no way for an existing account to create another organisation: one is created only together with a new account (at registration, or when a platform admin creates a user). **The owner wants all three built.**
  ...
  - **First, settle S1–S15 with the owner, before writing any code — S2, the spending guard, first.** Every organisation without a budget of its own gets the $25 default, and nothing caps the total (B-14), so letting an account create organisations freely multiplies what one account can spend. The guard must count the organisations an account has **created**, not those it owns now: an Owner can promote another member to Owner and then leave, which `backend/routers/organisations.py` allows while another Owner remains, so a cap on owned organisations would reset whenever two accounts pass ownership around. The proposal is therefore `MAX_ORGANISATIONS_CREATED_PER_USER`, default 3, counted from a new `created_by` column on `organisations` (the table has none today), with platform admins exempt (Codex's review of pull request #7). None of S1–S15 is decided until the owner says so.
  ...
  - **Not to be missed** (details in the plan): a public share page needs an allow-list of fields, because `_public_product` (`backend/routers/products.py`) returns the whole project row — `email_settings` and `company_details` included, only the SMTP password left out; an unknown, expired or revoked share token must answer 404, never 401, because a 401 makes the frontend renew the session and then sign the visitor out; live updates are per organisation, with no `product_id` in their events, so per-project access must add it; and a project-only guest needs an organisation context: `/api/auth/me` returns only the user's memberships and `AuthProvider` (`frontend/src/lib/auth/AuthProvider.tsx`) picks the organisation for `X-Org-Id` from that list, so `access.membership()` (`backend/services/access.py`) would refuse a guest before any project check — the profile must list project grants, each with its project's organisation, and `access.membership()` and the frontend's organisation context must represent guest access without granting membership (Codex's review of pull request #7).
- **Next** (priority order; the full lists are in `docs/ROADMAP.md`):
  1. **Sharing**, the active task above.
  2. **T-4 (owner), at the end of every session:** rotate the Railway project token. This is the owner's settled practice: `railway login` will not authorise on this machine, so the owner pastes a project token into the session and rotates it at the end of every session; the owner reported doing so at the end of the 2026-09-19 session. Never write a token value into a file, a document or a commit message: CI runs gitleaks over the full git history.
  3. Optional (T-5 to T-8): turn on Wait for CI in the `launchops` service's source settings, turn on database backups, delete the leftover volume `postgres-volume-qVKY`, set `MAIL_*`.
  4. Owner decisions still open: B-1 to B-12 — plan section 8, the brand kernel questions in D15, whether organisation owners should also create reset links (D8), and billing (B-12), which matters most because B-14's markup reasoning depends on it — and B-16, the sharing decisions S1–S15, which are settled first, with the active task.
  5. Product work already chosen (`docs/ROADMAP.md` → Next up): cost and duration per operation, which needs `result_id` on `ai_usage`; the Portfolio launch board as cards on a phone (LIM-003); the project tab bar on a phone (LIM-004).
  6. Phase 2 follow-ups: brand kernel (D15), result history (D16), billing settings.
  7. Phase 3: real actions.
...
- **Doc version:** 1.2.1 — this is the `version:` in this file's frontmatter, not a separate number. All seven managed documents carry that one shared version and are raised together.
...
- **Never run sign-up or registration probes against the live site.** `ADMIN_EMAIL` only guards the *first* account; once the admin exists, a probe creates a real account. One did on 2026-09-17 (`guard-check@example.com`), and the owner had to delete it by hand (T-1, done 2026-09-19). Test registration locally or against the test suite.
...
- **Status (2026-09-19):** live. `launchops` answers at https://launchops.run over HTTPS and at https://launchops-production-0457.up.railway.app. **Production runs `199121c`'s application code, from the pull request #6 merge**; `c8da4f0`, the 1.2.0 handoff merge, and this 1.2.1 update carry documentation only. It was checked from the live site, not in the Railway dashboard, whose deployment record was not read. From 2026-09-18T19:09:32Z, about a minute after the merge, the site served the bundle `index-P8sStBDZ.js`: its Settings → Usage chunk (`UsageSettings-J658VHHI.js`) contains `08705de`'s and `b607021`'s wording ("and the most an organisation can set", "As a platform administrator, you can set a higher one", "…can't start again until next month.") and no longer pull request #5's note naming a platform administrator ("only a platform administrator can"); its Operations chunk (`OperationsPage-1azU3X9R.js`) contains the playbook's text ("Working out what's next", "Always available"); and the call to `/api/queue/summary` is in a shared chunk (`Display-D2gG6dyx.js`). `GET /health` answered 200. Re-checked on 2026-09-19, and again at 2026-09-19T16:32:19Z, after the 1.2.0 handoff merged as `c8da4f0`: the same bundle, `index-P8sStBDZ.js`, and `GET /health` 200, so production's application code is still `199121c`'s. Before that, production ran `477eaa4`, the pull request #5 merge, checked the same way: from 2026-09-18T04:43:44Z the site served `index-D1A5232b.js`, whose Settings → Usage chunk had all of pull request #5's wording and, when re-checked at 2026-09-18T06:09:38Z, none of pull request #6's. On 2026-09-19 the owner did T-10's signed-in check on the live site and reported that it looks good — the Operations screen opens on the playbook, and Settings → Usage is right. That is the owner's report; the assistant does not sign in. Checked on 2026-09-17 against the deployment of `bc6143c` (the pull request #3 merge), and not repeated since:
