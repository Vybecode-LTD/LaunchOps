...
document: TESTING
version: 1.2.1
last-updated: 2026-09-19T17:01:56Z
...
Per-file counts were first taken from the test files on 2026-09-17 (`pytest --collect-only`, `vitest list` plus the `it.each` tables, `playwright test --list`). At `b607021`, the last code commit on pull request #6, every file's Defined count in section 4 was checked against its source again without running anything, and the Run counts of the files pull request #6 changed were recounted from their `parametrize` and `it.each` tables and the two browser specs' screen lists. All agree with the runs. Pass counts and coverage come from the full runs of 2026-09-18 at `b607021`, one suite at a time, and were measured again on `main` at `199121c`, pull request #6's merge, on 2026-09-19, with identical results: only documents differ between the two commits.

## At a glance

| Suite | Location | Runner | Latest runs: 2026-09-18 at `b607021`, and again on `main` at `199121c` on 2026-09-19 |
|---|---|---|---|
...
**Total: 1,321 tests, all passed in both runs. There are no xfails or skips.**

The other gates were clean at `b607021`, and again on `main` at `199121c`: `ruff check .`, `npm run lint` (zero warnings) and `npm run typecheck` (`tsc -b`). The dependency audits last ran on 2026-09-16/17 — pip-audit found no known vulnerabilities, and npm's advisory service none in 614 installed package versions (checked from Python; see section 2) — and pull request #6 changed no dependency. **CI on pull request #6** passed all three jobs on `08705de`, again on `b607021`, its last code commit (run 35312501264, green by 2026-09-18 05:57:07 UTC), and on its last commit, `705c834`, the 1.1.4 documentation (green by 06:43:43 UTC); `4af499c` has no run of its own. It merged into `main` as `199121c`, a merge commit, at 2026-09-18 19:08:32 UTC. **Pull request #5** passed every job and step at `7dbc35d` (run 35298972693, 2026-09-18 02:20–02:26 UTC: ruff, pip-audit and pytest with the coverage gate; the gitleaks history scan; npm audit, ESLint, typecheck, Vitest with the coverage gate, the build and Playwright) and all three jobs again on its last commit, `ce12024` (green by 03:38:11 UTC), then merged into `main` as `477eaa4`. **CI on `main`** passed on the pushes of both merges, `477eaa4` (run 35308026206) and `199121c` (run 35384260825), and on the push of `c8da4f0`, the 1.2.0 handoff merge (pull request #7, which changed only documents; run 35454992066, all three jobs green by 2026-09-19 16:32:04 UTC). An earlier push to `main`, `0ce65dd`, failed on gap 13's flake.
...
**On this development machine, run the suites one at a time**, never two at once: overlapping runs go red in ways that prove nothing (gap 14). Run Vitest at `--maxWorkers=2` (gap 14) and Playwright at `--workers=4` (gap 13). CI is unaffected.
...
- **Default DSN:** `DEFAULT_TEST_DSN` in `backend/tests/conftest.py`, currently `postgresql://postgres@127.0.0.1:56432/launchops_test?sslmode=disable` (port 56432), which is the local cluster below. Set `TEST_DATABASE_URL` to use a different server. CI sets `postgresql://postgres@localhost:5432/launchops_test?sslmode=disable` for its `postgres:18` service.
- **The local cluster, since 2026-09-19, is `%LOCALAPPDATA%\LaunchOps-dev\postgres`** (outside both OneDrive and Temp). It was created with `initdb -U postgres -A trust -E UTF8 --no-locale` (PostgreSQL 18.4) and listens on **56432**, the default in `conftest.py` and in `backend/.env`'s `DATABASE_URL`. It holds two databases: `launchops_test` for the test suite, and `launchops` for the local dev server. `launchops` is empty — the backend runs the migrations when it starts — and has no accounts, so using it means signing up on the local app; never probe sign-up on the live site. The server's log is `%LOCALAPPDATA%\LaunchOps-dev\postgres.log`.
- **It is stopped at session start, and after any reboot,** so `python -m pytest` can't connect until it is started (the commands follow this list). Once it is up, `python -m pytest` needs no `TEST_DATABASE_URL`: the 2026-09-19 run on `main` used it that way, 570 passed, 98.33%.
- **Why it moved:** the earlier clusters lived in an earlier session's temp scratchpad, `%LOCALAPPDATA%\Temp\claude\C--DEV-LaunchOps\<session-id>\scratchpad\pgdata` (port 56432) and `...\scratchpad\pgdata2` (port 56433). Windows' temp clean-up removed their empty directories, which PostgreSQL needs even when they are empty, and on 2026-09-19 `pgdata2` refused to start: "could not open directory pg_replslot: No such file or directory", and the same for `pg_tblspc`. Both are broken, and abandoned rather than deleted; don't start them. The 2026-09-17 19:0x and 2026-09-18 runs used the one on 56433 (section 5, History).
- **The always-on service on port 5432 is not a substitute:** its `postgres` user needs a password that is recorded nowhere in this project (`psql` there answers `fe_sendauth: no password supplied`).
- **Two pytest runs must never share a test database.** Every test empties all app tables, and `test_migrations.py` creates and drops databases on the same server, so a second run on the same cluster fights the first. On 2026-09-17 a second agent's run against this machine's default cluster produced deadlocks and foreign-key violations that read like real failures and were not. With one local cluster, run one backend suite at a time; a second run at the same moment needs a server of its own through `TEST_DATABASE_URL`, created outside Temp and OneDrive.

Starting, checking and stopping the local cluster, from Git Bash:
...
"/c/Program Files/PostgreSQL/18/bin/pg_isready.exe" -h 127.0.0.1 -p 56432     # "accepting connections" once it is up
"/c/Program Files/PostgreSQL/18/bin/pg_ctl.exe" -D "$LOCALAPPDATA/LaunchOps-dev/postgres" stop -m fast
```

**Redirecting the start command's output to `/dev/null` is what stops it hanging.** Without the redirect, the call hangs on the shell's pipe, which is why `pg_ctl` used to look hung.
...
npm run coverage -- --maxWorkers=2            # the same, as run locally on this machine (gap 14)
...
- **No backend needed:**
  - `e2e/support/fakeBackend.ts` routes every browser `**/api/**` request through MSW's `getResponse`. It passes on only the `authorization` and `content-type` headers, so `X-Org-Id` is dropped and every browser test acts in the signed-in user's first organisation (gap 17).
...
- **The fake backend** (`src/test/fakeApi.ts`) is an in-memory model of the backend contract:
  - **Organisations and roles:** requests act on the organisation in `X-Org-Id` (or the user's first) and need the backend's minimum role for each change, refused with the backend's message. Members, invitations, the activity log, sessions (renewal and sign-out) and password reset links are modelled too. **Project data is scoped more weakly than in the backend** (gap 17): only `GET /api/products` filters by organisation, and single projects and results are found by id alone.
  - **Results:** `GET /api/queue/summary` counts one project's results by operation and status, as the backend route does, and answers 404 for an unknown project, but it finds the project by id alone, so another organisation's project is counted where the backend answers 404 (gap 17). `GET /api/queue` filters by project and status but ignores `limit` and returns results in the order they were added, where the backend returns the newest first and at most `limit` (gap 15).
...
| `frontend/e2e/support/fakeBackend.ts` | `useFakeBackend(page, state)` and `requestsTo()` for Playwright. Answers `/api/events` with `retry: 5000` and ends, because Playwright can't stream a routed response. Passes on only the `authorization` and `content-type` headers, not `X-Org-Id` (gap 17) |
...
## 5. Coverage snapshot (2026-09-18 at `b607021`, and again on `main` at `199121c` on 2026-09-19)

### Backend (pytest-cov, statement coverage of application code)

**98.33%**: 3,356 statements, 56 missed, at `b607021` and again on `main` at `199121c`, which has the same code. `backend/.coveragerc` leaves out the tests and virtual environments, and the run fails below 95%.
...
- **Result:** 99 passed at `b607021`, and again on `main` at `199121c`: 3 smoke, 12 golden path, 56 accessibility (28 screens × light and dark) and 28 responsive (the same 28 screens at 390×844). "All operations", added by pull request #6, is the newest screen.
- **Browser:** locally, for both runs, the pre-installed headless shell through the temporary config (section 2), deleted afterwards, at `--workers=4`: at the default 8 workers, two full runs each had a different phone-width screen time out (gap 13). CI installs Chromium.
...
| 2026-09-18, pull request #6 at `b607021` | 570 passed | 98.33% application code (3,356 statements, 56 missed) | 652 in 51 files | 97.13 / 90.24 / 96.59 / 99.19 | 99 passed |
| 2026-09-19, `main` at `199121c` (latest) | 570 passed | 98.33% application code (3,356 statements, 56 missed) | 652 in 51 files | 97.13 / 90.24 / 96.59 / 99.19 | 99 passed |
...
The 2026-09-19 run re-verified `main` at `199121c`, pull request #6's merge (2026-09-18 19:08:32 UTC, a merge commit, not a squash). Its code is exactly `b607021`'s: `git diff 705c834 199121c` is empty, and only documents differ from `b607021`, so no test was added or changed. Every figure was measured one suite at a time and matches the `b607021` run's: the backend with `ruff check .` clean and `python -m pytest --cov` run without `TEST_DATABASE_URL`, against the new local cluster on port 56432 (section 2); the frontend with ESLint at zero warnings, `tsc -b` clean and `npm run coverage -- --maxWorkers=2`; and Playwright at `--workers=4` through the temporary local-browser config, deleted afterwards. Both 95% gates still pass.
...
3. **No browser test runs against the real backend.**
   ...
   - The fake mirrors backend rules by hand: template tags, approval-to-draft rules and contact extraction, role minimums, the daily email limit, usage summaries, the budget stop and which budget applies (`effectiveBudget`, a copy of `effective_budget()`), who may set a budget above the platform default or lower one already above it (a copy of `ensure_budget_allowed()`), and the results summary (`GET /api/queue/summary`).
     ...
     - Not mirrored: name guessing from nearby words, the result list's `limit` and newest-first order (gap 15), and the backend's organisation scoping of project data (gap 17).
   ...
   - **`GET /api/queue/summary`, new in pull request #6, is checked by reading both sides rather than by a test that spans them.** The frontend's request (`/api/queue/summary?product_id=`) and the fields it reads (`product_id`, `workflow_id`, `status` and `count`, in `QueueSummaryRow`) were read against `backend/routers/queue.py` and match, and the route itself is covered against real Postgres by `test_queue.py` and `test_tenancy.py`. A signed-in check against a local backend wasn't done: the assistant doesn't create accounts or enter passwords. The owner did the signed-in check of the deployed playbook on the live site on 2026-09-19 (T-10 in `docs/ROADMAP.md`) and reported that it looks good: the Operations screen opens on the playbook, and Settings → Usage is right. It was a one-off manual check, reported by the owner rather than seen by the assistant, and covers the code as it was on 2026-09-19 (`199121c`'s application code); no automated test spans the two sides.
...
13. **Browser tests flake under CPU contention; first seen on `main` and investigated 2026-09-17, seen again 2026-09-18, cause not proved.** Recorded so the work isn't repeated.
   ...
   - **The earliest occurrence was on `main` itself**, before the `a2124a9` failure, and was first recorded here in 1.2.0: the push of `0ce65dd`, pull request #4's merge (run 35269583667, 2026-09-17 20:14 UTC), failed the frontend job at `e2e/golden.spec.ts:105`, the same test, with the same "Clicking the checkbox did not change its state". Pull request #5's CSS changes, listed below, weren't in `0ce65dd` — they came after it (`git diff 0ce65dd a2124a9`) — so the flake is older than the branch it was first seen on. `908f46d` made the test patient (below), and **every push to `main` since has passed**: the merges of pull request #5 (`477eaa4`, run 35308026206) and pull request #6 (`199121c`, run 35384260825).
   ...
   - **Seen again on 2026-09-18, on other screens** (pull request #6 at `b607021`). `playwright.config.ts` sets no `workers`, so a local run starts half the logical processors — 8 of this machine's 16 — and at that count two full local runs each had a different phone-width screen time out after 10 s waiting for the screen to render: "Run sheet", then "Calendar month". Both passed on their own ("Run sheet" 5 of 5 with `--repeat-each 5`; the whole responsive spec 28 of 28), and the full suite passed, 99 of 99, at `--workers=4`. Pull request #6 changed neither `RunSheet.tsx` nor the calendar page (the run sheet opens over the Operations screen, which it did change). A different screen each time, each passing alone, fits CPU contention rather than a defect in either screen; as before, the cause isn't proved. **Run the full suite locally with `--workers=4`** (section 2). CI is unaffected. The full run on `main` at `199121c` on 2026-09-19, at `--workers=4`, passed 99 of 99.
...
17. **The fake backend scopes project data by organisation only in part, and the Playwright bridge drops `X-Org-Id`.** Read in the code at `199121c` on 2026-09-19. The backend scopes every project, result, draft and list to the request's organisation and answers 404 for another's; the fake (`frontend/src/test/fakeApi.ts`) does so only in part:
   - **Only `GET /api/products` filters by organisation**, and it keeps projects without an `org_id`, so they show in every organisation.
   - **The queue, the Outbox, templates, captures, the calendar and brands aren't filtered by organisation.** `GET /api/queue` and `GET /api/email-queue` filter only by project (and the queue by status); `GET /api/templates`, `/api/captures`, `/api/calendar` and `/api/brands` return everything in the state.
   - **`GET /api/products/:id` and `GET /api/queue/summary` look the project up by id only** (the fake's `project()` helper); `PATCH /api/products/:id`, `DELETE /api/products/:id` and `PATCH /api/queue/:id` likewise act on a row found by id alone. So the fake's summary answers 200 for another organisation's project, where the backend answers 404. That is CodeRabbit's review thread on pull request #6 at `fakeApi.ts:709`, answered and left open. It is test-only: the backend is right, as `test_queue.py::test_summary_needs_a_project_in_the_organisation` and `test_tenancy.py::test_queue_items_are_isolated` show against real Postgres.
   - **The Playwright bridge drops `X-Org-Id`.** `e2e/support/fakeBackend.ts` passes on only the `authorization` and `content-type` headers, so the fake falls back to the user's first organisation and browser tests always act in it.
   - **So no frontend test can catch one organisation's project data showing in another's.** Organisation isolation is tested only in the backend (`test_tenancy.py`, and the cross-organisation cases in `test_organisations.py`). The fake does filter the organisation-level lists by organisation: members, invitations, the activity log and usage.
   - **To be fixed first in the sharing work** (`docs/SHARING_PLAN.md`, "How to test it"). Before relying on frontend tests there, make the fake filter every list by organisation (and, for per-project access, by project) and look projects up within the organisation, which closes pull request #6's open thread, and make the bridge forward `X-Org-Id`.
