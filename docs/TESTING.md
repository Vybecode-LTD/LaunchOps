...
Per-file counts were taken from the test files on 2026-09-17 (`pytest --collect-only`, `vitest list` plus the `it.each` tables, `playwright test --list`). The three test files changed by `6b97da3`, `43ab3e6`, `a53b26e` and `7dbc35d` were recounted from their source rather than collected, and the totals agree with the runs. At `7dbc35d`, every file's Defined count in section 4 was checked against its source again, without running anything, and all agree. Pass counts and coverage come from the full runs on 2026-09-16 and 2026-09-17.

## At a glance

| Suite | Location | Runner | Latest run, 2026-09-16/17 |
|---|---|---|---|
| Backend API, database, jobs and AI client | `backend/tests/` | pytest | 564 collected, 564 passed |
| Frontend units, components, whole app | `frontend/src/**/*.test.ts(x)` | Vitest (jsdom) | 50 files, 628 passed |
| Browser, production build | `frontend/e2e/*.spec.ts` | Playwright (Chromium) | 96 passed |

**Total: 1,288 tests, all passed. There are no xfails or skips.**

The other gates passed in the same runs: `ruff check .`, `npm run lint` (zero warnings) and `npm run typecheck`. pip-audit found no known vulnerabilities, and npm's advisory service found none in 614 installed package versions (checked from Python; see section 2). At `7dbc35d`, the head of pull request #5, CI passed every job and step (run 35298972693, 2026-09-18 02:20–02:26 UTC): ruff, pip-audit and pytest with the coverage gate; the gitleaks history scan; and npm audit, ESLint, typecheck, Vitest with the coverage gate, the build and Playwright.
...
- **`e2e/a11y.spec.ts`** runs axe-core with the tags `wcag2a`, `wcag2aa`, `wcag21a`, `wcag21aa` and `wcag22aa`:
  - on 27 screens, in light and dark themes, with reduced motion;
...
- **Backend:** 338 plain tests + 226 cases from 31 parametrized tests = 564. The largest expansions are in `test_organisations.py` (59 cases), `test_workflows.py` (51), `test_results.py` (36) and `test_scraper.py` (23).
- **Vitest:** 535 plain tests + 93 cases from 6 `it.each` tables = 628. The tables: `endpoints.test.ts` 58, `results.test.tsx` 12 + 12 + 5 + 3, `fakeApi.test.ts` 3. `npx vitest list` shows 541, because it lists each `it.each` once.
...
| `src/lib/api/endpoints.test.ts` | 4 | 61 | Every API helper sends the right method, path and body (58 cases, `it.each`); the live updates stream at `GET /api/events`; empty filters left out of list addresses; undoable deletes use keepalive only when told to |
...
| `src/lib/domain/usage.test.ts` | 10 | 10 | Usage months in UTC, stepping across year ends, never a malformed or future month, month names; dollars to the cent (tiny costs shown as under a cent), grouped counts, operation names from the catalogue; budget warning from 80% and used up from 100%; `parseBudget` amounts and error messages |
...
**Vitest total: 541 defined, 628 run in 50 files.**
...
## 5. Coverage snapshot (2026-09-16/17, latest run)

### Backend (pytest-cov, statement coverage of application code)

**98.32%**: 3,340 statements, 56 missed. `backend/.coveragerc` leaves out the tests and virtual environments, and the run fails below 95%.
...
Between 95% and 100%: `routers/organisations.py` 95.41%, `main.py` 95.83%, `routers/queue.py` 96.02%, `services/usage.py` 97.37%, `database.py` 97.66%, `services/scraper.py` 97.71%, `services/access.py` 98.25%, `routers/auth.py` 98.31% and `services/claude.py` 98.33%.
...
2. **Branch coverage is below statements and lines, and nothing gates it.**
   - Frontend: branches 90.13% (statements 97.08%, functions 96.53%, lines 99.21%). Only lines are gated.
...
3. **No browser test runs against the real backend.**
   ...
   - The fake mirrors backend rules by hand: template tags, approval-to-draft rules and contact extraction, role minimums, the daily email limit, usage summaries, the budget stop and which budget applies (`effectiveBudget`, a copy of `effective_budget()`), and who may set a budget above the platform default (a copy of `ensure_budget_allowed()`).
     ...
     - Name guessing from nearby words is not mirrored.
...
13. **A browser test flaked under CPU contention; investigated 2026-09-17, cause not proved.** Recorded so the work isn't repeated.
...
14. **The Vitest suite isn't reliable at default concurrency on this development machine — measured 2026-09-17; the cause is likely, not proved.** `npm run check` and `npm run coverage` go red here with no change at all, so **a red frontend run is not by itself a regression**: re-run the failing file on its own, or the suite with `--maxWorkers=2` (`npx vitest run --maxWorkers=2`, or `npm run coverage -- --maxWorkers=2`), before believing it. This goes further than gap 13's warning about CPU contention: it happens on a quiet machine at the default worker count, not only under heavy parallel load.
   ...
   - **Measured on a quiet machine.** Before measuring, every `node` process was checked by command line: all were `@modelcontextprotocol/server-pdf` MCP servers belonging to other sessions, and no test runner of any kind was running.
     - **This branch, full `npx vitest run`, 3 runs:** 618 passed; 3 failed; 2 failed.
     - **`main`'s frontend source, 4 runs:** 2 failed; 7 failed; 1 failed; 1 failed. **4 of 4 runs red, so the flakiness is pre-existing on `main`** and wasn't introduced by this session's changes. Method: `git checkout main -- frontend/src`, then restored, with `frontend/src` confirmed identical to `HEAD` afterwards. That checkout leaves this branch's two new playbook files in place, but nothing else imports them.
   ...
   - **Process lesson:** never let two documentation agents run the test suites at the same time. This session did, and it produced a batch of red runs that proved nothing.
