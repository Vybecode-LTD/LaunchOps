...
- `main` is at **`f143f1c`** (merge of PR #2). Every push to `main` deploys.
- **Uncommitted at handoff:** `CLAUDE.md`, `backend/routers/auth.py`, `backend/tests/test_sessions.py`, `backend/tests/test_claude.py`, `docs/ASSESSMENT_AND_DEVELOPMENT_PLAN.md`, `docs/TESTING.md`, `SOURCE_MAP.md`, `SETUP_PROMPT.md`, and the untracked session-end documents `docs/BUGS.md`, `docs/HANDOFF.md`, `docs/ROADMAP.md`, `docs/AUDIT-LOG.md` (with `docs/CHANGELOG.md` still to be written) — plus the permanently untracked `.github/workflows/test-pipeline.yml`. **Whether to commit this batch is still open:** the owner has not been asked yet.
...
All four P1 items are account/DNS/console housekeeping, not code. IDs are from `docs/ROADMAP.md` → Active.
...
Nothing below moves without a decision; each is written up with its working default in `docs/ROADMAP.md` → Blocked (B-1 … B-13).
...
- **This session's uncommitted work** (B-13) — ask before committing, and branch first (see "Where the project stands").
...
- **Merge pull requests with merge commits, never squash or rebase.** `.gitleaksignore` entries are per-commit fingerprints (`commit:file:rule:line`); rewriting hashes breaks them and the secret scan starts failing on already-reviewed findings.
...
| Repo | GitHub **`Vybecode-LTD/LaunchOps`**, branch `main`, deploys on every push |
...
| `docs/ROADMAP.md` | Milestones, Active tasks (T-1…T-8), Blocked (B-1…B-13), Next up, Backlog |
