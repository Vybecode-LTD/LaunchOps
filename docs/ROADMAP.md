...
| B-13 | Commit the session-end batch — the BUG-027 refresh-token fix and this handoff's documents — to `main`? | Session end 2026-09-17; BUG-027 in `docs/BUGS.md` | Nothing is pushed without the owner asking. When asked: branch off `main`, open a pull request, wait for CI, then merge with a **merge commit** — never a squash, because `.gitleaksignore` pins its three reviewed findings by per-commit fingerprint. The batch is `backend/routers/auth.py`, `backend/tests/test_sessions.py`, `backend/tests/test_claude.py`, `CLAUDE.md`, `SOURCE_MAP.md`, `SETUP_PROMPT.md`, `docs/ASSESSMENT_AND_DEVELOPMENT_PLAN.md` and `docs/TESTING.md`, plus this handoff's untracked documents in `docs/`. It carries the BUG-027 fix, so **that security fix is not live until the batch merges**; `main` is at `f143f1c` and every push to it deploys to production. |
