...
| Backend tests | 570 passed; 98.3% coverage of application code (gate 95%). |
| Frontend lint and type check | Passing: ESLint with zero warnings, strict TypeScript. |
| Frontend tests (Vitest) | 652 passed in 51 files; 99.2% line coverage (gate 95%). Fails intermittently at default concurrency on the development machine, pre-existing on `main`, so these figures come from runs at `--maxWorkers=2` — see `docs/TESTING.md` gap 14. |
| Browser tests (Playwright) | 99 passed: 3 smoke, 12 golden path, axe WCAG 2.2 A/AA scans of 28 screens in both themes, and 28 screens checked at a 390px phone width. Locally, run with `--workers=4`: at the default 8, one phone-width screen can time out under load (`docs/TESTING.md` gap 13). |
...
2. Finish the deployment. It's live on Railway (2026-09-17): project "Launch Ops" has `launchops` (built from `main` of `Vybecode-LTD/LaunchOps`, deploying every push; the worker runs inside it) and `Postgres`, and answers at https://launchops-production-0457.up.railway.app. `DATABASE_URL`, `JWT_SECRET`, `FIELD_ENCRYPTION_KEY`, `APP_URL`, `ADMIN_EMAIL` and `ANTHROPIC_API_KEY` are set.
   ...
   **Live smoke test (2026-09-17):** `generate_result` was run against the real API with the deployment's key, and all three calls returned valid results (about $0.10 in total):
   ...
   Still to do (the full list, with IDs, is `docs/ROADMAP.md` → Active, T-1 to T-10):
   - **done 2026-09-18:** pull request #5 merged (`477eaa4`, a merge commit) and deployed (T-9). It fixed three defects that were
     live in production — organisations without a budget could spend without limit on the deployment's API key, four screens
